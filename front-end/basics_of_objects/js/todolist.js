; (function () {
	...
	function Task(name, completed = false, createdAt = Date.now(), updatedAt = null){
        ...
        if(!name) throw new error("Task need a required parameter: name");
        let _name = name
    ...
    let arrInstancesTasks = arrTasks.map(task => new Task(task.name, task.completed, task.createdAt, task.updatedAt))
